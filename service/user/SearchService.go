...
type SearchServiceData1 struct {
	model.Curriculums
	Number string 					`gorm:"column:number" json:"number"`  // todo 坑 集合查询映射的字段类型是string而不是int
...
func SearchService(c *gin.Context) *serialize.Response{
	...
	totalSql := "select " +
				...
				"where " +
					...
					"c.c_name like '%"+key+"%' or t.t_name like '%"+key+"%' group by c.c_id) count;"
	...
	DB.DB.Raw(searchSql,start,num).Scan(&data.Result)

	// 查询数据的总和
	DB.DB.Raw(totalSql).Count(&data.Total)
